...
function getApi(pCtx: IPluginContext): IFcaQroubat | null {
  const primary = pCtx.consumeService<IMiraiQroubat>("mirai-transport")?.getApi?.() ?? null;
  if (primary) return primary;
  return pCtx.consumeService<IMiraiQroubat>("mirai-transport-secondary")?.getApi?.() ?? null;
}
